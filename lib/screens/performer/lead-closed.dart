...
class LeadClosedScreen extends StatefulWidget {
  const LeadClosedScreen({super.key});

  @override
  State<LeadClosedScreen> createState() => _LeadClosedScreenState();
}

class _LeadClosedScreenState extends State<LeadClosedScreen> {
  ...
  Future<void> initializeData(stageId) async {
    ...
    try {
      await Future.wait([fetchStages(stageId)]);
    } catch (e) {
  ...
  Future<void> fetchStages(stageId) async {
    String langCode =
        Provider.of<LanguageProvider>(context, listen: false).languageCode;
    final api = ApiService();
    try {
      final response = await api.post('/api/performer/lead/stage/close', {
        'stageId': stageId,
      });
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        // Title
                        Text(
                          getText("Lead Successfully Closed!", langCode),
                          style: Theme.of(
                            context,
